...
if [%2]==[] (
  ...
  if exist %module%_alternative.runconfig (
    :: specific runconfig exists in third line of runconfig
    for /F "skip=2 delims=" %%i in (%module%_alternative.runconfig) do if not defined files set "files=%%i"
  ) else (
  ...
  if exist %module%_alternative.runconfig (
    :: specific runconfig exists in fourth line of runconfig
    for /F "skip=3 delims=" %%i in (%module%_alternative.runconfig) do if not defined files set "files=%%i"
  ) else (
    echo No alternative runconfig provided.
    exit /b 1
  )
