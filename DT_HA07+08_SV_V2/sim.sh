...
if [ $# -eq 1 ]; then # no further args => default mode
  ...
  if test -f "${module}_alternative.runconfig"; then # test if specific alternative runconfig exists
    files=`cat ${module}_alternative.runconfig | head -n 3 | tail -n 1` # use third line of runconfig
  else
    echo No alternative runconfig provided.
    exit 1
  fi
elif [ "$2" = "altgrdsol" ]; then # => alternative grading on solution mode (testing purposes)
  if test -f "${module}_alternative.runconfig"; then # test if specific runconfig exists
    files=`cat ${module}_alternative.runconfig | head -n 4 | tail -n 1` # use fourth line of runconfig
  else
    echo No alternative runconfig provided.
    exit 1
  fi
